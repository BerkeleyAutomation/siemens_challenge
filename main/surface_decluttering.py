...
class SurfaceDeclutter():
    ...
    def grasp_all(self, c_img, d_img, bboxes):
        ...
        for bbox in bboxes:
            ...
            d_img_focused = self.focus_on_target_zone(d_img, bbox)
            depth_image_mm = np.asarray(d_img_focused[:,:])
            depth_image_m = depth_image_mm/1000
            plt.imshow(depth_image_m)
            plt.show()
            plan_start = time.time()
            from autolab_core import RigidTransform, YamlConfig, Logger
            from perception import BinaryImage, CameraIntrinsics, ColorImage, DepthImage, RgbdImage
    ...
    def quantify_sensor_noise(self):
        d_img_array = np.empty((30,480,640))
        ...
        #std[std > 60] = 0
        np.save('/home/benno/sensor_noise_hsr_final_std.npy', std)
        np.save('/home/benno/sensor_noise_hsr_final_depth.npy', d_img_array)
        std[std == 0] = None
        plt.figure()
        for p in xrange(0,5):
...
if __name__ == "__main__":
    ...
    while number_failed <= 2:
        ...
        if task.TARGET_DIR is not None:
            file = open(task.TARGET_DIR + 'timing_analysis.txt', 'a')
