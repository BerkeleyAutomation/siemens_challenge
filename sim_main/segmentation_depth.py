...
def find_item_masks(folder_path):
	masks = []
	bkgr = cv2.imread(folder_path+"/depth_background.png", 0)
	with open(folder_path+"/"+"labels.json") as f:
		lst = json.load(f)
	# print(lst)
	item_num = len(lst)
	depth = [reduce_noice(cv2.threshold(find_curr_img_mask(cv2.imread(folder_path+"/depth_"+str(i)+".png", 0), bkgr)[1], 10, 255, 0)[1]) for i in range(item_num)]
	for i in range(item_num):
		cv2.imwrite(folder_path+"/debug_"+str(i)+".png", depth[i])
	

	for i in range(item_num):
		curr_img = cv2.imread(folder_path+"/depth_"+str(i)+".png", 0)

		diff = depth[i]

		for j in range(i):
			prev_depth = depth[j]
			shift_prev, diff = find_curr_img_mask(diff, prev_depth)
			
		masks.append(reduce_noice(diff))

		# cv2.imwrite(folder_path+"/debug_"+str(i)+".png", depth_scaled_to_255(diff))
...
def draw_masks(folder_path):
	...
	# print(img)
	diff = cv2.subtract(cv2.imread(folder_path+"/depth_"+str(item_num - 1)+".png",0), cv2.imread(folder_path+"/depth_background.png", 0))
	pure_img = extract_mask(diff)
